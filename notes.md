...
### Postgres
- Serial option is choosen when we want something to incrementally change.
- select * from products - fetches all the columns from the table
- select id, name from products - only fetches id and name from the table
- select id AS products_id from products - fetches the id column but under the name products_id
- select * from products WHERE id = 10
- select * from products where price >= 80;
- select * from products where inventory != 0;
- Combining operators: select * from products where inventory > 0 and price > 20;
- Combining operators: select * from products where price > 100 OR price < 20;
- IN operator: SELECT * FROM products WHERE id IN (1, 2, 3);
- LIKE operator: select * from products WHERE name LIKE 'TV%';
- ORDER By operator: select * from products order by price DESC;
- ORDER by operator using two factors: select * from products order by inventory desc, price
- LIMIT: select * from products where price > 10 LIMIT 2;
- OFFSET: select * from products order by id limit 5 offset 5 - skips the offset no of records from the starting
